...
for topic, search_url in search_urls.items():
    ...
    # get page count
    nb_pages = int(driver.find_element_by_class_name(
        'qa-pagination-total-page-number').text)
    #print(f'Topic "{topic}" has {nb_pages} result pages')
    # for each page
    for page_index in range(nb_pages):
        # scrape the links
        link_list = driver.find_element_by_tag_name(
            'ol').find_elements_by_tag_name('li')
        for entry in link_list:
            try:
                link = entry.find_element_by_tag_name(
...
url_df = url_df[url_df.url !=
                r'http://www.bbc.co.uk/faqs/questions/bbc_online/sharing']
...
for url_index, url in enumerate(article_urls.url):
    ...
    if(not response.ok):
        print(
            f'Failed request: {url_index} : {url}, result: {response.status_code}')
        continue
    ...
    for div_soup in div_list:
        ...
        if(data_component != 'text-block'):
            continue
        text_ps += [clean_text(html_to_text_converter.handle(str(p_soup)))
                    for p_soup in div_soup.find_all('p')]
    text = '\n'.join(text_ps)
    if(len(text) == 0):
...
climate_phrases = ['climate', 'global warming', 'carbon dioxyde',
                   'renewable', 'CO2', 'coal', 'carbon', 'rising temperatures', ]

article_mention_counts = {phrase: sum(
    articles.text.str.lower().str.find(phrase) > 0) for phrase in climate_phrases}
